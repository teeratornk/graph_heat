"""
Create a script that loads the split and stats, then performs min-max normalization
(train-only scalers) for the consolidated GIS dataset.

Inputs:
- full_dataset.hdf5  # consolidated, filtered, deduped data
- train_val_test_split.json  # contains {"train_ids": [...], "test_ids": [...], "val_time_idx": [...], "train_time_idx": [...]}
- processing_report.json  # shapes/stats; if stats are global (not train-only), recompute train-only min/max

Normalization goals (fit scalers on TRAIN ONLY):
- Per-feature min-max scalers (with epsilon for zero-range) for:
  1) topology: t, x, y, z
     - t: from the effective values used (after dropping first frame). If not provided by report, derive from train_time_idx (e.g., 0..119).
     - x, y, z: per-dimension min/max over all nodes used by train samples.
  2) node features:
     - k
     - (optional) q, s if used later
  3) edge feature:
     - edge_attr_r
  4) globals:
     - I (current), T (ambient) — min/max across the train sample set
  5) targets:
     - temperature

Normalization formula (apply train-fitted scalers to train/val/test uniformly):
- x_norm = (x - min_x) / max(1e-12, (max_x - min_x))
- Clip to [0,1] after transform to avoid numerical spillovers.
- DO NOT normalize integer indices (edge_index) or categorical labels.

Script tasks (step-by-step):
1) Load train/val/test split:
   - Read train_val_test_split.json.
   - Extract: train_ids (list of sample IDs), test_ids, train_time_idx (list of ints), val_time_idx (list of ints).
   - Validate: 0 <= all time idx < 120; no overlap between train_time_idx and val_time_idx.

2) Load processing_report.json:
   - If it contains per-field min/max already computed on TRAIN ONLY, use them.
   - Otherwise, recompute min/max from full_dataset.hdf5 restricted to TRAIN:
     a) For topology:
        - t: min/max from train_time_idx (e.g., min(t_train), max(t_train)).
        - x,y,z: take all nodes' node_pos for each train sample if per-sample differs;
          if /shared/node_pos exists, use that shared array. Compute per-dimension min/max across all train samples/nodes.
     b) Node features:
        - k: if /shared/k present and same across samples, compute on shared; else aggregate across train samples. Exclude any nodes removed earlier.
        - (optional q,s: same pattern if needed later; prepare scalers if requested.)
     c) Edge features:
        - edge_attr_r: if shared, compute on shared; else aggregate across train samples.
     d) Globals:
        - I_T: for each train sample, collect I and T; min/max per dimension.
     e) Targets:
        - temperature: aggregate all values from train samples and train_time_idx across nodes; compute min/max.

3) Save scalers (train-only) to a separate JSON file:
   - scalers.json with structure:
     {
       "topology": {
         "t": {"min": ..., "max": ...},
         "x": {"min": ..., "max": ...},
       ...
       "node": {
         "k": {"min": ..., "max": ...},
         "q": {"min": ..., "max": ...},    # include only if used
         "s": {"min": ..., "max": ...}     # include only if used
       },
       "edge": {
         "r": {"min": ..., "max": ...}
       },
       ...
       "meta": {
         "fit_on": "train_only",
         "train_ids": [...],
         "train_time_idx_range": [min_train_t, max_train_t]
       }
     }

4) Apply normalization to produce a normalized copy of the dataset:
   - Create normalized_full_dataset.hdf5 with same structure as full_dataset.hdf5.
   - Copy over /shared arrays unchanged in shape; apply normalization to:
       • /shared/node_pos[:, 0..2] → x,y,z dims independently, using scalers["topology"]
       • /shared/k → using scalers["node"]["k"]
       • /shared/edge_attr_r → using scalers["edge"]["r"]
     NOTE: If an array is not shared and exists per sample, normalize per sample using the same (train-only) scalers.
   - For /globals/I_T (num_samples, 2): normalize each column with scalers["globals"]["I"], ["T"].
   - For each /samples/S***/temperature (120, N, 1):
       • Normalize all values using scalers["target"]["temperature"].
   - For each /samples/S***/topology (120, N, 4):
       • Normalize t,x,y,z columns using the corresponding scalers. If original topology stored t as absolute indices, transform to [0,1] via t scaler.

5) Keep edge_index intact (no normalization) and keep dtypes:
   - edge_index: int64
   - All normalized reals: float32
   - Preserve shapes; write attributes in HDF5 noting "normalized=True".

6) Write split metadata into normalized HDF5:
   - Under /meta/suggested_split store the JSON from train_val_test_split.json.
   - Under /meta/scalers store the contents of scalers.json as a JSON string (duplicate).

7) Print a concise summary:
   - Which arrays were normalized and their shapes.
   - A few sanity checks (min≈0, max≈1 for each normalized field over TRAIN; note that VAL/TEST might be slightly outside [0,1] if out-of-range).
   - Confirm no changes to edge_index counts.

Implementation details:
- Use: h5py, json, numpy.
- Robustness:
  - If max==min for any scaler, set denom to 1 and output all zeros (log a warning).
  - If processing_report.json is missing or does not contain train-only stats, recompute from HDF5 as described.
  - Set a fixed seed for any stochastic ops (though normalization itself is deterministic).

Optional flags (as function args or CLI):
- --include-q-s  (if True, also compute/apply scalers for q and s; otherwise ignore)
- --in  full_dataset.hdf5
- --out normalized_full_dataset.hdf5
